...
function GOJSTypeInIt(side) {
    ...
    GOJS.template(GOJS.myPalette, [function() {}, function() {}]);
    ...
    GOJS.myPalette.model = go.Model.fromJson(typeModleJson);
}
...
function GOJSInit(side) {
    ...
    GOJS.template(GOJS.myDiagram, [GOJS.mClick, GOJS.cClick]);
    GOJS.loopNode();
    GOJS.loopLink(); // 动态特效开始执行
    // GOJS.isReadOnly(true)// 默认只读
}

GOJS.cClick = function(e, obj) {
    let node = obj.part.data;
    if (!node.pathId) {
        return false;
    }
    vm.historyPages.push({
        projectVal: vm.projectVal,
        chartId: vm.chartId
    });
    vm.projectVal = node.projectVal;
    vm.$http
        .post(
            "/S-Base/getConfigurationByStructureId",
            Qs.stringify({ StructureId: node.projectVal[node.projectVal.length - 1] })
        )
        .then(res => {
            if (res.data.success) {
                ...
                vm.chartId = node.pathId;
                vm.$http
                    .post(
                        "/S-Base/getConfigurationById",
                        Qs.stringify({ id: node.pathId })
                    )
                    .then(res => {
                        if (res.data.success) {
                            ...
                            GOSJLoad(res.data.data[0].content, true);
...
//   if (obj.part.data.category == 'datatitle') {
//     GOJS.thisNode = obj.part.data // 当前双击选中的图形节点数据预留，暂时无用
//     vm.$http.post('/S-RealData/getRealDataByPK', Qs.stringify({
//       id: GOJS.thisNode.id
//     })).then(res => {
//       if (res.data.success) {
//         if (res.data.data.readWrite === '写') {
//           vm.dialogFormVisible = true
//           vm.form.title = GOJS.thisNode.vname || GOJS.thisNode.nname
//           vm.form.nodeId = GOJS.thisNode.id
//           if (GOJS.thisNode.enums && GOJS.thisNode.enums.length > 0) {
//             vm.form.dataType = 'list'
//             vm.enumsOptions = GOJS.thisNode.enums
//           } else {
//             vm.form.dataType = 'text'
//           }
//           if (GOJS.thisNode.enums_value) {
//             vm.form.value = GOJS.thisNode.enums_value
//           } else if (GOJS.thisNode.value !== null && GOJS.thisNode.value !== undefined) {
//             vm.form.value = GOJS.thisNode.value
//           } else if (GOJS.thisNode.vname !== null && GOJS.thisNode.vname !== undefined) {
//             vm.form.value = GOJS.thisNode.vname
//           } else if (GOJS.thisNode.nname !== null && GOJS.thisNode.nname !== undefined) {
//             vm.form.value = GOJS.thisNode.nname
//           } else if (GOJS.thisNode.name !== null && GOJS.thisNode.name !== undefined) {
//             vm.form.value = GOJS.thisNode.name
//           }
//         } else {
//           // vm.$message.error('不可修改')
//         }
//       } else {
//         vm.$message.error(res.data.msg)
//       }
...
GOJS.template = function(obj, pa) {
    obj.nodeTemplateMap.add(
        ...
        GOJS.$GO(
            go.Node,
            "Auto", { doubleClick: pa[0], click: pa[1] },
            GOJS.nodeStyle(), {
...
function GOJSSaveNode() {
    ...
    GOJS.myDiagram.model.updateTargetBindings(node);
}
/**
 * 保存当前编辑的图形
                ...
                if (data.success) {
                    ...
                    for (var k in node) {
                        if (datakey[node[k]["devDetailed"]]) {
                            ...
                            node[k]["id"] = datakey[node[k]["devDetailed"]]["_id"];
                            // if (node[k].stroke == undefined || node[k].stroke == null) {
                            //     node[k].stroke = '#409EFF'
                            // }
